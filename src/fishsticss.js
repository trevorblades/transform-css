...
var fishsticss = {
  _explode: function(selectorRaw) {
	  return selectorRaw.split(',');
  },
  _scrape: function(css , commentMap) {
    ...
    while (match) {
      ...
      if (commentMap[match.index] == undefined){
	      ...
	      var selectorRaw = match[1].trim();
	      //remove any comments 
	      var cssStyles = match[2].trim();
	      var commentFiltered = cssStyles.replace(COMMENT_PATTERN, '\n');
	
	      //if the selector contains multiple values then we need to explode it
	      var exploded = this._explode(selectorRaw);
	      for(var selectorIndex in exploded){
	    	  var selector = exploded[selectorIndex].replace(COMMENT_PATTERN, '\n').trim();
  ...
  prepare: function(css) {
    ...
    while (match) {
    	if (commentData.commentMap[match.index] == undefined){
    		...
    		var matchedCss = matchedMediaQuery.slice(matchedMediaQuery.indexOf("{")+1).substring(0, matchedMediaQuery.length-1);
    		//Not in a comment
